...
import Aux from '../../../hoc/Aux';
  ...
  const items = [
    {type:'pizza', label: 'Festival', ingredients: 'tomato sauce, cheese, tomato, bazil', price: '17,99'},
    ...
    {type:'burger', label: 'Standard', ingredients: 'beef chop, cheese, tomato, cucumber, onion, bbqsauce', price: '19,99'},
    ...
    {type:'cookie', label: 'Chocolate', price: '7,99'},
  ...
  const builders = (props) => (
    <Aux>
      <div>
       <p> Pizza </p>
        {items.map(item => (
          <Builder key={item.label} type={item.type} label={item.label} ingredients={item.ingredients} price={item.price}
